 ...
 * of a graph that are independent both of its type as of their
 * implementation.<br>
 *
...
public abstract class Graph {
    ...
    /**
     * If it isn't in the graph, adds the edge (i, j) with weight w to a Weighted
     * graph.
     *
     * @param i Source vertex
     * @param j Target vertex
    ...
    protected boolean bfsSpanningTree(int origin, Edge[] res) {
        ...
        while (!q.isEmpty()) {
            ...
            for (l.begin(); !l.isEnd(); l.next()) {
                Adjacent a = l.get();
                if (visited[a.target] == 0) {
                    res[visitOrder++] = new Edge(u, a.target, a.weight);
                    visited[a.target] = 1;
                    q.enqueue(a.target);
                }
            }
